...
const navigate = useNavigate();
    ...
    const handleClick = async ()=>{
        ...
        if(password!=="" && confPassword!==""){
          if(password===confPassword){
            try {
        const response = await fetch("https://pneuexpress.online/foody/resetpassword.php", {
          method: "POST",
          headers: {
...
<input
  ...
  style={{
    border: error1?'1px solid red':password ? '1px solid #fb9300' : '1px solid rgba(251, 147, 0, 0.2)',
    marginBottom: '20px'
  }}
  type='password'
...
<input
  ...
  style={{
    border: error2?'1px solid red':confPassword ? '1px solid #fb9300' : '1px solid rgba(251, 147, 0, 0.2)',
    marginBottom: '20px'
  }}
  type='password'
