  ...
  useEffect(()=>{
    ...
    const createTicket = async ()=>{
      const payload = {
        ...
        if(!errorTicket){
        try {
          const res = await fetch("https://pneuexpress.online/foody/createTicket.php", {
            method: "POST",
            headers: {
    ...
    const addMessageToTicket = async (e)=>{
      ...
      const payload = {
        ...
        if(!errorTicket){
        try {
          await fetch("https://pneuexpress.online/foody/addMessageToTicket.php", {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
            },
...
function deg2rad(deg) {
  ...
  try {
    const res = await fetch("https://pneuexpress.online/foody/updateCourrierInfo3.php", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
...
const fetchTickets = useCallback(async () => {
  try {
    const response = await fetch("https://pneuexpress.online/foody/getTickets.php", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
...
const pickUp = async (orderId,customerId,restoName) => {
  try {
    const response = await fetch("https://pneuexpress.online/foody/pickUpOrder.php", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
...
const markAsDelivered = async (orderId,customerId,restoName) => {
  try {
    const response = await fetch("https://pneuexpress.online/foody/markAsDelivered.php", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
 ...
 useEffect(() => {
  ...
  }, [donut,total_orders_per_restaurant,analytics]); // ← re-run when `analytics` changes
  ...
  const handleImageClick = () => {
    ...
    try {
      const res = await fetch("https://pneuexpress.online/foody/updateCourrierInfo.php", {
        method: "POST",
        body: formData,
      });
...
formData.append("lng", String(position.lng));
  ...
  try {
    const res = await fetch("https://pneuexpress.online/foody/updateCourrierInfo2.php", {
                    method: 'POST',
                    body: formData
    });
  ...
  const fetchData = useCallback(async()=>{
      try {
        const response = await fetch("https://pneuexpress.online/foody/getCourrier.php", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
        ...
        if(result.userData.photo!==null)
          setSrc(`https://pneuexpress.online/foody/${result.userData.photo}`)
      } catch (error) {
...
const isOrderInCourierRange = useCallback((orderObj, position, range) => {
...
const fetchOrders = useCallback(async () => {
  try {
    const response = await fetch(
      "https://pneuexpress.online/foody/getOrdersCour.php",
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
    ...
    const deliveredOrders = result.orders.filter(
      elm => parseInt(elm.order.courrierId) === parseInt(id) && elm.order.status === "Delivered"
    );
  ...
  ;
    ...
    const activeOrders = result.orders.filter(
      elm => parseInt(elm.order.courrierId) === parseInt(id) && elm.order.status === "Active"
    );
...
},[id,isOrderInCourierRange,position,range]);

    const [days,setDays] = useState(null)
    const [numOrders,setNumOrders] = useState(null)
    const getStats= useCallback(async()=>{
      try {
        const response = await fetch("https://pneuexpress.online/foody/statsCourrier.php", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
 ...
 const handleClick = ()=>{
  ...
  return (
    <div>
      ...
      {addTicket && <div className='addticketpopup2'>
      <div style={{display:'flex',justifyContent:'space-between'}}><h1>Create a ticket</h1><i style={{cursor:'pointer',fontSize:'1.2em'}} onClick={()=>{document.body.style.overflow='unset';setOverlay(false);setAddTicketPopUp(false)}} className="fa-solid fa-xmark"></i></div>  
        ...
        <div style={{marginTop:'20px'}}><span onClick={createTicket} style={{cursor:'pointer',color:'white',backgroundColor:'#fb9300',padding:'10px 20px'}}>Create</span></div>
      </div>}
      ...
      <div style={{display:'flex',justifyContent:'space-between'}}><h1>Ticket messages</h1><i style={{cursor:'pointer',fontSize:'1.2em'}} onClick={()=>{document.body.style.overflow='unset';setOverlay(false);setMessagesTickets(false)}} className="fa-solid fa-xmark"></i></div> 
        ...
        <div className='kolo'><span onClick={addMessageToTicket}>Send</span></div>
      </div>}
...
{success7 && <p ref={message7} className='message' style={{textAlign:'center',zIndex:'10005',position:'fixed',top:'0',width:'100%',backgroundColor:'#bfe9d3',border:'1px solid #008d00'}}>Success!</p>}
      ...
      <div ref={menu} id='slam'>
            ...
            <div id='links' style={{width:'100%',textAlign:'left',fontSize:'1.3em',display:'flex',flexDirection:'column'}}>
                ...
                <p style={{display:'flex',alignItems:'center',width:'100%',backgroundColor:tickets?'#ffecd1':"",color:tickets?"#fb9300":""}} onClick={()=>{setAnalytics(false);setDelivery(false);setCredentials(false);setTickets(true);setProfile(false);}}><i className="fa-solid fa-bars"></i> Support</p>
    ...
    return (
      <div key={index} className='deli'>
        ...
        <div style={{ textAlign: 'center', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center' }}>
          ...
          <img alt="aeaze" src="https://21985162c3f6de69b3a2fa38c4458a89.cdn.bubble.io/cdn-cgi/image/w=384,h=384,f=auto,dpr=1,fit=contain/f1645091940786x269735977037557380/compressedcart.png"/>
    ...
    return (
      ...
      <div className='fff'>
       ...
       {openMapIndex === index && (
          ...
          <img alt="eazeza" src="https://21985162c3f6de69b3a2fa38c4458a89.cdn.bubble.io/cdn-cgi/image/w=384,h=384,f=auto,dpr=1,fit=contain/f1645091940786x269735977037557380/compressedcart.png"/>
          <p style={{fontWeight:'bold'}}>There are no data here yet</p>
          <p style={{color:'#aaa',fontSize:'1.3em'}}>Looks like there are no new things. Please visit again later.</p>
      </div>)
}
    ...
    return (
      ...
      <div className='fff'>
       ...
       {openMapIndex2 === index && (
          ...
          <img alt="eaze" src="https://21985162c3f6de69b3a2fa38c4458a89.cdn.bubble.io/cdn-cgi/image/w=384,h=384,f=auto,dpr=1,fit=contain/f1645091940786x269735977037557380/compressedcart.png"/>
          <p style={{fontWeight:'bold'}}>There are no data here yet</p>
          <p style={{color:'#aaa',fontSize:'1.3em'}}>Looks like there are no new things. Please visit again later.</p>
      </div>)
}
    ...
    return (
      <div key={index} className='deli'>
        ...
        <div style={{ textAlign: 'center', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center' }}>
          ...
          <img alt="ezaeaz" src="https://21985162c3f6de69b3a2fa38c4458a89.cdn.bubble.io/cdn-cgi/image/w=384,h=384,f=auto,dpr=1,fit=contain/f1645091940786x269735977037557380/compressedcart.png"/>
          <p style={{fontWeight:'bold'}}>There are no data here yet</p>
          <p style={{color:'#aaa',fontSize:'1.3em'}}>Looks like there are no new things. Please visit again later.</p>
      </div>)
}
