...
function CusDash() {
    ...
    }, [filteredCategories2, search]); // include all values used inside the function
  ...
  if (!auth) {
    ...
    const fetchNotifications= useCallback(async()=>{
      try {
        ...
        result.notifications.forEach((elm)=>{
          if(elm.flag===1){
...
useEffect(() => {
  ...
  fetchRestaurants();
  fetchMeals();
}, [fetchRestaurants]); // empty dependency array: runs only once
...
useEffect(() => {
  ...
  try {
   ...
   const selectCategory = (elm) => {
      const selectedId = parseInt(elm.target.id);
    ...
    <div id='main'>
      ...
      {success34 && <p ref={message34} className='message' style={{textAlign:'center',zIndex:'5100',position:'fixed',top:'0',width:'100%',backgroundColor:'#bfe9d3',border:'1px solid #008d00'}}>Email sent!</p>}
        ...
        {form && <div className='form'>
            ...
            <i onClick={()=>{document.body.style.overflow='unset';setOverlay(false);setForm(false)}} id='close1' className="fa-solid fa-xmark"></i>
  ...
  cartBuy?.length > 0 ? (
    ...
    <h2>Order Menu</h2>
      {cartBuy.map((elm, index) => (
        <div
          key={`${elm.mealIndex}-${elm.mealName}`}
            ...
            <div id="restaurants">
              {nearbyRestaurants.length>0 ? nearbyRestaurants.map((elm,index)=>(
                ...
                <div  style={{margin:'20px',padding:'50px',display:'flex',flexDirection:'column',backgroundColor:'white',boxShadow:'0 0 2px #ddd',borderRadius:'15px'}}>
                  ...
                  <Link to={`/restoinfo/${elm.id}`}><i style={{color:'white',backgroundColor:'#fb9300',padding:'10px',borderRadius:'60%'}} className="check fa-solid fa-caret-right"></i></Link>
                  </div>
                </div>
                </div>
              )):<div style={{display:'flex',alignItems:'center',flexDirection:'column'}}>
                <img id='no' src='https://21985162c3f6de69b3a2fa38c4458a89.cdn.bubble.io/cdn-cgi/image/w=384,h=384,f=auto,dpr=1,fit=contain/f1645091940786x269735977037557380/compressedcart.png' alt='photoa'/>
                <div style={{display:'flex',justifyContent:'center',alignItems:'center',flexDirection:'column'}}>
                  <h3>There are no restaurants matching your criteria in your area yet.</h3>
...
{filteredMeals && filteredMeals.length>0 && <h1 style={{fontSize:'1.3em'}}>Most Ordered Meals 🤩</h1>}
                 <div id="meals">
              {filteredMeals && filteredMeals.map((elm,index) => (
        ...
        <div style={{padding:'40px'}}>
            ...
            <div style={{display:'flex',width:'100%'}}>
                <div style={{backgroundColor:'white',padding:'30px',border:'1px solid #DDD',margin:'30px',width:'50%'}}>
                    ...
                    {!previewUrl2 && <img onClick={handleImageClick2} style={{padding:'2px', cursor: 'pointer',border:'2px dashed #ccc', borderRadius: '10px'}} width='120' src={src2==null?'':src2} alt="Click to upload"/>}
                    <input
                        type="file"
                        ref={fileInputRef}
