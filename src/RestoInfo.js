...
const [numComments,setNumComments] = useState(false)
  ...
  const fetchNotifications= useCallback(async()=>{
      try {
        ...
        const hasFlag = result.notifications.some(elm => elm.flag === 1);
        setFlagNotification(hasFlag);
     ...
     const flag= async(id)=>{
      try {
        await fetch("https://pneuexpress.online/foody/flag.php", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
    ...
    const clear= async()=>{
      try {
        await fetch("https://pneuexpress.online/foody/clear.php", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
...
useEffect(() => {
  fetchNotifications();
  fetchComments();
}, [fetchComments, fetchNotifications]);
    ...
    if(localStorage.getItem("allowedResto")==null){
      ...
      if(cart.length===0){
        ...
        ////cart)
          ...
          if(parseInt(localStorage.getItem('allowedResto'))!==id_resto){
          ////localStorage.getItem("allowedResto"),id_resto)
...
const updateQuantity = (index, quantity) => {
  ...
  setNumberOfItems(number);
    ...
    const addFavorite = async ()=>{
      try {
        const response = await fetch("https://pneuexpress.online/foody/addFavorite.php", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
    ...
    const deleteFavorite = async ()=>{
      try {
        const response = await fetch("https://pneuexpress.online/foody/deleteFavorite.php", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
...
useEffect(()=>{
  ...
  setNumberOfItems(number)
    ...
    useEffect(()=>{
        ...
        fetchMeals()
        fetchData2()
    },[fetchData,fetchMeals,fetchRes,fetchData2])
...
style={{ border: 'none', outline: 'none', height: '20px', width: '60px', }}
    ...
    <div id='fall'>
      ...
      <p>anything to your cart yet</p>
        ...
        <div id='pop'>
            <img alt="" style={{marginRight:'20px',borderRadius:'50%',objectFit:'cover'}} width='150' height='150' src={res.logo!==null?`https://pneuexpress.online/foody/${res.logo}`:'https://21985162c3f6de69b3a2fa38c4458a89.cdn.bubble.io/cdn-cgi/image/w=64,h=,f=auto,dpr=1,fit=contain/f1643645120337x435152378012765760/chefauchef-01%2B%281%29.png'}/>
        ...
        <div id='com'>
          <Link to='#'><span style={{color:'#fb9300',fontSize:'1.3em',marginRight:'40px'}}>Menu</span></Link>
          <Link to={`/comments/${id}`}><span style={{fontSize:'1.3em'}}>Comments({numComments})</span></Link>
        </div>
        <div id='iop'>
                <h1 id='fo'>Most Ordered Meals 🤩</h1>
                 <div id="meals">
              {meals && meals.map((elm,index)=>(
                <div key={index}>
                  <img style={{borderTopLeftRadius:'20px',borderTopRightRadius:'20px',objectFit:'cover'}} height='200' width='300' src={`https://pneuexpress.online/foody/${elm.photo}`} alt={`meal ${index}`}/>
            ...
            <div>
           {meals && categories.map((cat, index) => {
  const catMeals = meals.filter(meal => parseInt(meal.category) === cat.id);

  if (catMeals.length === 0) return null;

  return (
    <div key={index} style={{ marginTop: '15px' }}>
      <h1 style={{ fontWeight: '500', fontSize: '1.5em', margin: '20px' }}>
        {cat.name} Meals
      </h1>

      <div style={{ display: 'flex', flexWrap: 'wrap' }}>
        {catMeals.map((mealItem, i) => (
          <div
            key={i}
            style={{
              width: 'fit-content',
          ...
          >
            <img
              ...
              height="200"
              width="300"
              src={`https://pneuexpress.online/foody/${mealItem.photo}`}
              alt={`meal ${i}`}
            />
            <span
              style={{
            ...
            >
              {mealItem.ingredients.split(' ').join(', ')}
            </span>
            <div
              style={{
                display: 'flex',
            ...
            >
              ...
              <i
                onClick={()=>addToCart(meals[meals.findIndex(meal => meal.id === mealItem.id)].id,mealItem.id_restaurantOwner)}
