...
function CustomerSignUp() {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if(file && name && surname && email && password && passconfirm){
            if(validatePasswords()){
                alert('The two passwords do not match!')
            }else{
                localStorage.getItem('courrierEmail') && localStorage.removeItem('courrierEmail');
                localStorage.setItem('courrierEmail', email);
                ...
                const res = await fetch('https://pneuexpress.online/foody/addCourrier.php', {
                    method: 'POST',
                    body: formData
                });
  ...
  return (
    <div id='main'>
      <header>
        ...
        <Link to='/login'><span><i className="fa-solid fa-right-to-bracket"></i> Login</span></Link>
      </header>
      <main id='mainn'>
        <div>
            <div style={{textAlign:'center'}}>
            <h1>Step 2:</h1>
            <p style={{fontSize: '1.2em',fontWeight: 'unset',marginBottom:'50px'}}>Register to Lunchbox</p></div>
            <form style={{wordBreak:'break-word'}} onSubmit={handleSubmit}>
                <label>Driver License</label><br/>
                <label htmlFor="file-upload" className="custom-file-upload">
                  {fileName}
                </label>
                <input name='file'  onChange={handleFileChange} id="file-upload" type="file" /><br/><br/><br/>
